...
#definition[
  Функция $f: X -> Y; X, Y in subset.eq BB^*$ называется *полиномиально вычислимой*, если существует полиномиальная (детерминированная) машина Тьюринга М такая, что
  #eq[
...
#definition[
  Функция $f: union_(n in NN) BB^n -> BB^*$ называется *сильно односторонней*, если
...
#definition[
  Функция $f: union_(n in NN) BB^n -> BB^*$ называется *слабо односторонней*, если
  + $f$ полиномиально вычислима
  +
  #eq[
...
#proof[
  ...
  Введём функцию
  #eq[
    $g(x_1, ..., x_t) := (f(x_1), ..., f(x_t)); quad x_i in B^n, t = n dot p(n)$
  ]

  Предположим, что $g$ -- не односторонняя, тогда для произвольного полинома $q$ существует п.в.м.Т. $B$ и бесконечное множество $N subset.eq NN$, что
  #eq[
    $forall n in N : mu_(x in_cal(U) BB^(n t))({B(1^(n t); g(x)) in g^(-1)(g(x))}) > 1 / q(n t)$
  ]
  ...
  Также определим вероятностный алгоритм $C$ на входе $y$, выполняющий алгоритм $C_0$ на этом входе не более $k := 2 dot n dot t dot q(n dot t)$ раз.
  ...
  #eq[
    $E_n = {x in BB^n | mu({C_0(1^n; f(x)) in f^(-1)(f(x))}) > n / k}$
  ]
  #lemma[
    #eq[
  ...
  #eq[
    $mu({C(1^n; f(x)) in f^(-1)(f(x))}) &>= \
      mu({A(1^n; f(x)) in f^(-1)(f(x))} | E_n)mu(E_n) &> (1 - e^(-n))(1 - 1 / (2 p(n)))$
  ]
